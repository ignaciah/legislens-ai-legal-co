...
# Load API keys from environment variables (create a .env file or set in terminal)
# For now, you can hardcode temporarily (but use .env for production!)
...
# Models
FLASH_MODEL = "gemini-2.0-flash"  # Fast extraction & translation
PRO_MODEL = "gemini-2.5-pro"     # Deep reasoning & strategy
...
def run_5_agent_pipeline(contract_text):
    ...
    # Use Pro model for deep reasoning
    ...
    try:
        ...
        # Fallback: wrap in JSON structure
        return {
            "risk_score": 50,
            "flags": [],
...
def translate_to_english(legal_text):
    """Agent 3: Translate legal jargon to plain English"""
    model = genai.GenerativeModel(FLASH_MODEL)
    prompt = f"""
    Translate this legal text to plain English (5th-grade reading level):
    {legal_text}
...
def get_chatbot_response(user_question, contract_context):
    ...
    Contract context:
    {contract_context[:3000]}  # Limit to avoid token overflow
